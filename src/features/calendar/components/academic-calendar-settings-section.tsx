...
import {
  activateTeacherAcademicYear,
  createTeacherAcademicYear,
  createTeacherSemester,
  listTeacherAcademicYears,
  listTeacherSemesters,
...
export function AcademicCalendarSettingsSection() {
  ...
  const loadYears = useCallback(async () => {
    const rows = await listTeacherAcademicYears();
    setYears(rows);
  ...
  const loadSemesters = useCallback(async (yearId: string) => {
    const rows = await listTeacherSemesters({ data: { academicYearId: yearId } });
    setSemesters(rows);
  }, []);
  ...
  const onCreateYear = async (e: React.FormEvent) => {
    ...
    setSavingYear(true);
    try {
      const created = await createTeacherAcademicYear({
        data: {
  ...
  const onActivateYear = async (yearId: string) => {
    ...
    try {
      await activateTeacherAcademicYear({ data: { academicYearId: yearId } });
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            أنشئ سنة دراسية نشطة وفصولها حتى يتمكن النظام من جدولة الحصص والتحضير.
          </p>
        </div>
          ...
          <>
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h4 className="text-xs font-bold text-slate-700">السنة الدراسية</h4>
                <Button
                  type="button"
                  variant="outline"
                    ...
                    return (
                      <li
                        ...
                        className={`rounded-xl border p-3 flex flex-col sm:flex-row sm:items-center gap-3 ${
                          selected ? "border-primary/30 bg-primary/5" : "border-slate-100 bg-white"
                        }`}
                      >
                        <button
                          type="button"
            ...
            <div className="space-y-3 border-t border-slate-100 pt-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h4 className="text-xs font-bold text-slate-700">الفصول الدراسية</h4>
                {selectedYearId && (
                  <Button
                    type="button"
                    variant="outline"
                    ...
                    <ul className="space-y-2">
                      {semesters.map((sem) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm font-bold text-slate-800">{sem.label}</span>
                              {current && (
