...
import {
  ...
  HOMEWORK_REPORTS_EMPTY_DESCRIPTION,
  HOMEWORK_REPORTS_EMPTY_TITLE,
  HOMEWORK_REPORTS_ERROR_TITLE,
  HOMEWORK_REPORTS_HINT,
  homeworkStatusLabel,
  REPORTS_FILTER_OPTIONS,
  submissionStatusLabel,
  type ReportsFilterKind,
} from "../services/homework-reports-ui.logic";
...
export function HomeworkReportsPanel() {
  ...
  const periodQuery = useHomeworkPeriodReport(appliedFilter);
  const detailQuery = useHomeworkDetailReport(selectedHomeworkId);

  const summaryItems = useMemo(
    () => (periodQuery.data ? buildHomeworkSummaryItems(periodQuery.data.summary) : []),
    [periodQuery.data],
  );
    ...
    <div className="min-w-0 space-y-4" dir="rtl">
      <Card>
        <CardContent className="flex flex-wrap items-center gap-3 p-5">
          ...
          <div className="min-w-0">
            <h2 className="text-lg font-bold">تقارير الواجبات</h2>
            <p className="mt-1 text-xs text-muted-foreground">
              ملخص قراءة فقط لواجباتك وتسليماتك ضمن الفترة. لا يُرسل معرّف المعلم من الواجهة.
            </p>
            {rangeLabel ? (
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {kind === "custom" ? (
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_1fr_auto]">
              <div className="space-y-1.5">
                <Label htmlFor="hw-reports-from">من</Label>
                <Input
                  id="hw-reports-from"
                  type="date"
                  className="min-h-11"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="hw-reports-to">إلى</Label>
                <Input
                  id="hw-reports-to"
                  type="date"
                  className="min-h-11"
          ...
          <p className="text-[11px] text-muted-foreground">{HOMEWORK_REPORTS_HINT}</p>
        </CardContent>
      </Card>
      ...
      {periodQuery.isPending ? (
        ...
        <EmptyState
          icon={BarChart3}
          title={HOMEWORK_REPORTS_ERROR_TITLE}
          description={
            periodQuery.error instanceof Error
              ? periodQuery.error.message
              : "حدث خطأ أثناء قراءة تقرير الواجبات."
          }
          action={
        ...
        />
      ) : !periodQuery.data || periodQuery.data.homework.length === 0 ? (
        <EmptyState
          icon={BarChart3}
          title={HOMEWORK_REPORTS_EMPTY_TITLE}
          description={HOMEWORK_REPORTS_EMPTY_DESCRIPTION}
        />
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">واجبات الفترة</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="hidden overflow-x-auto rounded-xl border md:block">
                <table className="w-full min-w-[640px] text-sm">
                  ...
                  <tbody>
                    {periodQuery.data.homework.map((row) => {
                      const selected = selectedHomeworkId === row.id;
                      return (
                        <tr key={row.id} className="border-t">
                          ...
                          <td className="px-3 py-3">
                            <Badge variant="outline">{homeworkStatusLabel(row.status)}</Badge>
                          </td>
                          <td className="px-3 py-3">
                            <Button
                              type="button"
      ...
      {selectedHomeworkId ? (
        detailQuery.isPending ? (
          ...
          <EmptyState
            icon={BarChart3}
            title="الواجب غير متاح"
            description="لا يمكن عرض هذا الواجب أو أنه لا ينتمي لحسابك."
          />
        ) : (
          <Card>
            <CardHeader className="pb-3">
            ...
            <CardContent className="space-y-4">
              ...
              {detailQuery.data.students.length === 0 ? (
                <p className="text-sm text-muted-foreground">لا توجد تسليمات لهذا الواجب.</p>
              ) : (
                <>
                  <div className="hidden overflow-x-auto rounded-xl border md:block">
                      ...
                      <tbody>
                        {detailQuery.data.students.map((row) => (
                          <tr key={row.studentId} className="border-t">
                            ...
                            <td className="px-3 py-3">
                              <Badge variant="outline">{submissionStatusLabel(row.status)}</Badge>
                            </td>
                            <td className="px-3 py-3">
