...
function CurriculumManagementPage() {
  ...
  const extractMutation = useMutation({
    mutationFn: (base64: string) => extractCurriculumFromPdf({ pdfBase64: base64 }),
    onSuccess: (data) => {
  ...
  const saveDraftMutation = useMutation({
    mutationFn: (data: CurriculumData) => saveCurriculumDraft(data),
    onSuccess: () => {
  ...
  const publishMutation = useMutation({
    mutationFn: (fileId: string) => publishCurriculum({ fileId }),
    onSuccess: () => {
  ...
  const archiveMutation = useMutation({
    mutationFn: (fileId: string) => archiveCurriculum({ fileId }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (fileId: string) => deleteCurriculumDraft({ fileId }),
    onSuccess: () => {
  ...
  const handleEditDraft = async (file: AdminFile) => {
    ...
    try {
      const lessons = await getAdminCurriculumLessons({ fileId: file.id });
      setReviewData({
        id: file.id,
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <Button
                  onClick={async () => {
                    ...
                    toast.promise(
                      (async () => {
                        const { fileId } = await saveCurriculumDraft(reviewData);
                        await publishCurriculum({ fileId });
                      })(),
                      {
